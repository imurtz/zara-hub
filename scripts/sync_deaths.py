...
def main():
    ...
    # القيمتان رُفعتا من 5 إلى 25/20 بعد إلغاء الجدولة الدورية (صار التشغيل يدوياً فقط بزر "مزامنة
    # الآن") — الفجوة بين نقرة وأخرى قد تطول ساعات أو أياماً بدل 5 دقائق، فنحتاج نافذة أوسع لالتقاط
    # أي حذف/تعديل حصل بالموقع خلال تلك الفجوة. اكتشاف "الخبر الجديد" نفسه غير متأثر بهذا الرقم أصلاً
    # (يفحص القائمة كاملة دائماً بغض النظر عن هذا الرقم)
    ap.add_argument("--window", type=int, default=25, help="عدد أحدث الأخبار المفحوصة كل تشغيل لاكتشاف المحذوف/الناقص")
    ap.add_argument("--refresh-latest", type=int, default=20, help="أحدث كم خبراً تُعاد قراءة صفحتها كل مرة لالتقاط ما استُكمل/تغيّر بالموقع")
